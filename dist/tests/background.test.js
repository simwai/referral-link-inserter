...
function createUrlRegex(base, path, queryParameters) {
    const basePattern = base.replaceAll('.', '\\.');
    const pathPattern = path.replaceAll('/', '\\/');
    return new RegExp(`^${basePattern}${pathPattern}\\/ref=nosim\\?.*tag=${encodeURIComponent(queryParameters.tag)}.*(&|$)`, 'i');
}
...
test('generateAmazonAffiliateLink should modify the URL with affiliate tag and add ref=nosim', async (t) => {
    const originalUrl = 'https://www.amazon.com/dp/B0BKC67D3V?tag=oldTag&linkCode=ogi&th=1&psc=1';
    const expectedRegex = createUrlRegex('https://www.amazon.com', '/dp/B0BKC67D3V', { tag: t.context.affiliateId });
    const modifiedUrl = await t.context.navigationHandler.generateAmazonAffiliateLink(originalUrl, t.context.affiliateId);
...
test('generateAmazonAffiliateLink should correctly update the affiliate tag for a proxy link', async (t) => {
    const originalUrl = 'https://www.amazon.de/s?field-keywords=apple+airpods+wireless+ear+buds+bluetooth+headphones+with+lightning+charging+case+included+over+24+hours+of+battery+life+effortless+setup+for+iphone&geniuslink=true';
...
test('handleNavigation should ignore non-amazon and non-genius URLs', async (t) => {
    ...
    t.is(t.context.lastUpdatedTabUrl, undefined, 'URL should not be modified for non-target URLs.');
});
test('handleNavigation should handle navigation for amazon URLs', async (t) => {
    ...
    await t.context.navigationHandler.handleNavigation(details);
    t.not(t.context.lastUpdatedTabUrl, undefined, 'URL should be modified for Amazon URLs.');
    t.not(t.context.lastUpdatedTabUrl, details.url, 'Amazon URL should be updated with new affiliate tag.');
});
test('handleNavigation should handle navigation for genius URLs', async (t) => {
    ...
    t.not(t.context.lastUpdatedTabUrl, undefined, 'URL should be modified for Genius URLs.');
    t.not(t.context.lastUpdatedTabUrl, details.url, 'Genius URL should be updated with new affiliate tag.');
});
test('handleNavigation should not modify URL if affiliate ID is not found', async (t) => {
    ...
    await t.context.navigationHandler.handleNavigation(details);
    t.is(t.context.lastUpdatedTabUrl, undefined, 'URL should not be modified if affiliate ID is not found.');
});
